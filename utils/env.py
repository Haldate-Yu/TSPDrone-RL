...
def create_test_dataset(
    ...
    #   fname = 'RL-TSP-len-100-n_nodes-11.txt'
...
class DataGenerator(object):
    def __init__(self,
        ...
        # create test data
        self.test_data, self.val_data = create_test_dataset(args)
...
class Env(object):
    ...
    def step(self, idx_truck, idx_drone, time_vec_truck, time_vec_drone, terminated):
        ...
        time_vec_truck[:, 0] = np.logical_and(np.less(time_step, t_truck),
                                              np.greater(time_vec_truck[:, 1], np.zeros(self.batch_size))) * idx_truck
        ...
        self.state[idx_satis, idx_drone[idx_satis]] -= np.equal(time_vec_drone[idx_satis, 1],
                                                                np.zeros(len(idx_satis))) * self.state[
                                                           idx_satis, idx_drone[idx_satis]]
        ...
        a = np.equal(
            np.greater_equal(time_vec_truck[:, 1], 0).astype(int) + np.equal(time_vec_drone[:, 1], 0).astype(int), 2)
        ...
        a = np.equal(np.equal(time_vec_drone[:, 1], 0).astype(int) + np.equal(self.sortie, 1).astype(int) + np.equal(
            self.returned, 1).astype(int), 3)
        b_s = np.where(a)[0]
        avail_actions[b_s, :, 1] += (self.combined_nodes[b_s] - self.combined_check[b_s]) * np.expand_dims(
            np.greater(self.state[b_s].sum(axis=1), 2).astype(int), 1) * np.ones([len(b_s), self.n_nodes])
        ...
        avail_actions[b_s, :, 0] += (self.combined_nodes[b_s] - self.combined_check[b_s]) * np.expand_dims(
            np.greater(self.state[b_s].sum(axis=1), 2).astype(int), 1) * np.ones([len(b_s), self.n_nodes])

        # if there is expected visit by drone to that customer node with sortie =0
        # don't make that node available to truck 
        a = np.equal(np.equal(self.sortie, 0).astype(int) + np.greater(time_vec_drone[:, 1], 0).astype(int) + np.equal(
            time_vec_truck[:, 1], 0).astype(int), 3)
        ...
        a = np.equal(
            np.equal(self.truck_loc, time_vec_drone[:, 0]).astype(int) + np.greater(time_vec_drone[:, 1], 0).astype(
                int) + np.equal(time_vec_truck[:, 1], 0).astype(int), 3)
        b_s = np.where(np.expand_dims(a, 1))[0]
        avail_actions[b_s, self.truck_loc[b_s], 0] = 1

        # make the current location of drone available to truck if its stuck there
        a = np.equal(np.equal(self.returned, 0).astype(int) + np.equal(time_vec_drone[:, 1], 0).astype(int) + np.equal(
            time_vec_truck[:, 1], 0).astype(int), 3)
        b_s = np.where(np.expand_dims(a, 1))[0]
        ...
        a = np.equal(np.equal(self.state.sum(axis=1), 1).astype(int) +
                     np.equal((avail_actions[:, :, 0] == avail_actions[:, :, 1]).sum(axis=1), self.n_nodes).astype(
                         int) +
        ...
        a = np.equal(np.equal(self.state.sum(axis=1), 1).astype(int) + np.equal(time_vec_drone[:, 1], 0).astype(
            int) + np.greater(time_vec_truck[:, 1], 0).astype(int) + np.equal(self.returned, 1).astype(int), 4)
        b_s = np.where(a)[0]
